...
void GoGenerator::generateEnum()
{
	...
	for (auto &it : program_->enums_.defs_)
	{
		...
		goFile_ << indent() << setInitialUpper(it->name_) << "S2I"
				<< "= map[string] int16 {" << std::endl;
		for (auto &inner : it->defs_)
		{
		...
		goFile_ << indent() << setInitialUpper(it->name_) << "I2S"
				<< "= map[int16] string {" << std::endl;
		for (auto &inner : it->defs_)
		{
			goFile_ << indent() << "" << setInitialUpper(inner) << " : "
					<< "\"" << setInitialUpper(inner) << "\""
					<< "," << std::endl;
		}
		goFile_ << indent() << "}" << std::endl;
...
void GoGenerator::generateStruct()
{
	...
	for (auto &it : program_->structs_.defs_)
	{
		...
		if (generateContext->ns_.name_ != "rpc")
		{
		...
		//fingerprint
		goFile_ << std::endl;
		goFile_ << indent() << "const " << it->name_ << "_"
				<< "strFingerprint=\"" << md5(it->getFingerPrint()) << "\"" << std::endl;
		...
		indent_up();
		goFile_ << indent() << "return " << it->name_ << "_"
				<< "strFingerprint" << std::endl;
		indent_down();
		goFile_ << "}" << std::endl;

		if (generateContext->ns_.name_ == "rpc")
		{
			goFile_ << indent() << "func (this *" << setInitialUpper(it->name_) << ") Serialize( P__ IProtocol){" << std::endl;
		}
		else
		{
		...
		if (generateContext->ns_.name_ == "rpc")
		{
			goFile_ << indent() << "func (this *" << setInitialUpper(it->name_) << ") DeSerialize( P__ IProtocol) error{" << std::endl;
		}
		else
		{
...
void GoGenerator::genServiceStub()
{
	...
	for (auto &it : program_->services_.defs_)
	{
		...
		{
			goFile_ << indent() << it_inner->name_ << "CallBack"
					<< " " << setInitialUpper(it_inner->name_) << "CallBackType" << std::endl;
		}
		indent_down();
		goFile_ << indent() << "}" << std::endl;
		goFile_ << std::endl;

		goFile_ << indent() << "func New" << setInitialUpper(it->name_) << "Stub"
				<< "(rpcChan *rpc.RpcChannel) *"
				<< setInitialUpper(it->name_) << "Stub"
				<< "{" << std::endl;
		indent_up();
		goFile_ << indent() << "stub := &" << setInitialUpper(it->name_) << "Stub"
				<< "{strFingerprint: " << fingerprintName
		...
		goFile_ << indent() << "func (this *" << setInitialUpper(it->name_) << "Stub) "
				<< "Dispatch(msg *rpc.RpcMsg) error {" << std::endl;
		indent_up();
...
void GoGenerator::genServiceProxy()
{
	...
	for (auto &it : program_->services_.defs_)
	{
		...
		goFile_ << indent() << "func New" << cName << "(rpcService *rpc.RpcService,"
				<< "obj " << ifName << ") *" << cName << " {" << std::endl;
		indent_up();
		...
		//dispatch
		goFile_ << indent() << "func (this *" << setInitialUpper(it->name_) << "Proxy) "
				<< "Dispatch"
				<< "(";
...
void GoGenerator::deSerializeField(DefType *t, const std::string &fieldName, const std::string &inner)
{
	...
	else if (t->is_array())
	{
		...
		goFile_ << indent() << "for " << count.str() << ":=0; "
				<< "int32(" << count.str() << ")<" << str.str() << "; " << count.str() << "++ {" << std::endl;
		indent_up();
